...
> 3. `beginPath()`가 제대로 호출되지 않아,
> 4. 새롭게 그린 B가 이전에 그렸던 A와 이어진 선으로 그려짐
...
#### 💥 새로운 문제 발생
위와 같이 `beginPath()` 호출 위치를 수정해 이전 제스처와 물리적으로 연결되는 문제는 해결했지만, <br>
다른 유형의 시각적 문제가 새롭게 발생하였습니다.

<br>

## 2. 새로운 제스처가 실시간으로 보이지 않는 문제
> 제스처를 그리고 있는 도중에는 선이 보이지 않다가, 그리고 나서야 선이 한꺼번에 나타나는 문제
...
- 제스처가 끝나면 `fadeOutCanvas()`가 호출되어 선이 점점 사라짐
- 이후 다시 제스처를 그리기 시작하면:
...
| 두 번째 제스처 | 기존 캔버스가 남아 있음 → 새로 만들지 않음 | `-3.19189e-16` | 선을 그리고 있음에도 화면에는 출력되지 않음 |
...
> 그리는 동안에는 보이지 않았습니다.
> 
> 그러나 사용자가 마우스를 떼는 순간, 이 함수가 실행되며
> `opacity`가 1로 되살아나고, 그동안 그려진 선이 잠시 보였다가
> 곧바로 다시 0까지 줄어들며 사라진 것처럼 보이는 현상이 발생한 것입니다.
> 
...
### 최종 해결 방법
문제의 원인은, 완전히 투명해진 기존 캔버스가 DOM에 남아 있었고, <br>
