...
> **제스처 이름** (ex: "N자 패턴") ㅡ **사이트 정보** (ex: oogle.com") ㅡ **실행할 기능 이름** (ex: "메일함 열기")
...
서비스는 다음과 같은 흐름으로 동작을 수행합니다:

1. "N자 패턴" 제스처 → "notion.so" + "스크롤 맨 위로" 매핑 정보 확인
2. 해당 기능 이름을 기반으로, 내부 동작 목록에서 대응하는 함수 탐색
3. runAction("스크롤 맨 위로", "notion.so") 함수 호출
4. 내부에서는 `scrollTop > 0`인 모든 DOM 요소를 찾아, `scrollTo({ top: 0 })`를 실행

> 이처럼 사용자의 단순한 제스처 입력만으로도 실제 브라우저 동작이 자연스럽게 이어지는 구조입니다.
